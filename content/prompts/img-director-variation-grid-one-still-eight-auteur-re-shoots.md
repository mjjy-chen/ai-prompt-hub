---
title: "导演变体网格：一景八导重拍"
title_en: "Director Variation Grid: One Still, Eight Auteur Re-Shoots"
description: "AI图片生成提示词：Create a single 3x3 grid image (square, 2048x2048, high detail).
The center tile (row 2, col 2) must..."
description_en: "AI image generation prompt: Create a single 3x3 grid image (square, 2048x2048, high detail).
The center tile (row 2, col 2) must..."
category: "音乐艺术"
tags:
  - 图片生成
...
创建一张单独的3x3网格图像（正方形，2048x2048，高细节）。
中心格（第2行，第2列）必须是上传的参考电影剧照原片，保持不变。不得重新诠释、重绘、重打光、调色、裁剪、重构图、风格化、锐化、模糊或以任何方式对其进行变换。必须保持与提供的原图完全一致。

导演检测规则
如果上传的电影剧照的导演是下列8位导演之一，那么该导演对应的格子必须是原始中心格的精确副本。
